...
(function($, _, u){
  ...
  u.getAsync = function(opts) {
    ...
    $.getJSON(url).done(function(data){
      var jsondata = jsyaml.safeLoad(atob(data.content)),
          hydrated = [];
...
(function($, _, us){
  ...
  us.prototype.getLegislatorFromFormUrl = function(url) {
    var legislators = _.filter(this.legislators, function(legislator) {
      return legislator.contactFormUrl === url || url.match(new RegExp('\\b' + legislator.data.name.last.toLowerCase() + '\\b')) !== null;
    });
    if (legislators.length > 1) {
      legislators = _.filter(legislators, function(legislator){
        return url.match(new RegExp(legislator.data.name.first.toLowerCase())) !== null;
      });
    }
  ...
  us.Legislator.prototype.contactFormUrl = function(){
    return this.currentTerm().contact_form;
  };
...
(function($, _, f){
  ...
  f.fieldMappings = {
    ...
    '$ADDRESS_STREET': '[name*="address"]:not([name*="2"], [name*="second"])',
    '$ADDRESS_STREET_2': '[name*="address"]:not([name*="1"], [name*="first"])',
    '$ADDRESS_CITY': '[name*="city"]',
    '$ADDRESS_STATE_POSTAL_ABBREV': '[name*="state"]',
    '$ADDRESS_COUNTY': '[name*="county"]',
    '$ADDRESS_ZIP5': '[name*="code"]:not([name*="4"], [name*="four"], [name*="extended"])',
    '$ADDRESS_ZIP4': '[name*="code"][name*="4"], [name*="code"][name*="four"], [name*="code"][name*="extended"]',
  ...
  f.repr = function(el){
    ...
    return label + ' ' + el.tagName + ': ' + Utils.getSelector(el);
  };
...
window.CCH || (CCH = function(){
  ...
  // templates
  this.templates = {}
  this.templates.layout = _.template('<style>#stepform label span { display: inline-block; width: 150px; } textarea { width: 95%; min-height: 250px;}</style><div class="cch-container"><div class="cch-target"></div></div>');
  this.templates.selectLegislator = _.template('<label>Select a legislator:</label> <select class="cch-select-legislator" id=""> <option value="#">--</option> <% _.each(legislators, function(legislator){ %> <option value="<%= legislator.contactFormUrl() %>"><%= legislator.repr() %></option> <% }) %> </select>  <style>.cch-container{position:fixed; top:0; width:100%; padding:1em; border-bottom:1px solid lightgray; background:white;} .target{width:500px; margin:0 auto;}</style>');
  this.templates.popup = _.template('<h1>Working on: <%= legislator.repr() %></h1><form action="" id="stepform"><label><span>Select a form:</span> </label><select name="" id="formselector_select"><option>--</option><% _.each(forms,function(form, i){ %><option value="<%= i %>"><%= Utils.getSelector(form) %></option><% }) %></select><br/></form><div id="controls"><button class="reset">Reset & Start Over</button> <button class="next" type="button">Next &raquo;</button></div>');
  ...
  this.templates.fieldMapping = _.template('<label style="display: inline-block;" data-selector="<%= Utils.getSelector(field) %>"><span><%= Utils.getSelector(field) %>:</span> <select class="fieldmapping" data-selector="<%= Utils.getSelector(field) %>"> <option value="">--</option><% _.each(_.keys(Forms.fieldMappings), function(prop){ %> <option value="<%= prop %>"><%= prop %></option> <% }) %> </select></label> <label><input type="checkbox" class="required-field" data-selector="<%= Utils.getSelector(field) %>" />*</label><br/>');
  ...
  this.templates.commit = _.template('<h2>All done!</h2><p>Copy the yaml from the textarea below and submit a pull request via a <a href="https://github.com/unitedstates/contact-congress/new/master/members?filename=<%= bioguide %>.yaml" target="_blank">new file on github</a>.</p><textarea class="result"><%= yaml %></textarea>');
});
...
CCH.prototype.initialize = function() {
  ...
  $.when(legislatorsDownloaded, sampleLetterDownloaded, constantsDownloaded).done(function(){
...
CCH.prototype.popupWindow = function() {
  return this.popup || (this.popup = (window.opener) ? window : open('about:blank', "cch", "width=500,height=500"));
}

CCH.prototype.popupTargetDiv = function() {
  var tgt;
  return this.popupTarget || (this.popupTarget = (
                              tgt = $(this.popupWindow().document.body).find('.cch-target').length) ?
                                tgt :
...
CCH.prototype.filterShouldBeChecked = function(i, el){
  if (this.popupTargetDiv().find('select[data-selector="' + Utils.getSelector(el) + '"]').val() === "[check]") {
    return el;
  }
};

CCH.prototype.filterShouldBeUnchecked = function(i, el){
  if (this.popupTargetDiv().find('select[data-selector="' + Utils.getSelector(el) + '"]').val() === "[uncheck]") {
    return el;
  }
};
